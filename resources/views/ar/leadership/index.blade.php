...
@section('breadcrumb')
    <div class="-intro-x breadcrumb mr-auto hidden sm:flex">
        ...
        <a href="" class="breadcrumb--active">All Leadership</a>
    </div>
@endsection
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        @if ($authUser->hasPermissionTo('create leadership'))
            <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                <a class="button text-white bg-theme-1 shadow-md mr-2" href="{{ route('admin.leadership.create') }}">
                    Create Leadership
                </a>
            </div>
        @endif
                ...
                @if ($leaderships->count() > 0)
                    @foreach ($leaderships as $leadership)
                        <tr>
                            ...
                            <td class="border-b text-center">
                                <div class="font-medium whitespace-no-wrap">{{ $leadership->title }}</div>
                            </td>
                            <td class="text-center border-b">
                                <a class="flex items-center text-theme-15" href="javascript:;" data-toggle="modal"
                                    data-target="#notice-content-{{ $leadership->id }}"> <i data-feather="eye"
                                        class="w-4 h-4 mr-1"></i> View Content </a>
                            </td>
                            <td class="border-b text-center">
                                <div class="font-medium whitespace-no-wrap">{{ $leadership->createdByEntity->name }}</div>
                            </td>
                            <td class="border-b text-center">
                                <div class="font-medium whitespace-no-wrap">{{ $leadership->created_at->format('d-M-Y') }}
                                </div>
                            </td>
                            <td class="border-b text-center">
                                {!! $leadership->status !!}
                            ...
                            <td class="border-b w-5">
                                <div class="flex sm:justify-center items-center">
                                    <a class="flex items-center mr-3 text-theme-9"
                                        href="{{ route('admin.leadership.edit', $leadership->id) }}"> <i
                                            data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
                                    <a class="flex items-center text-theme-6" href="javascript:;" data-toggle="modal"
                                        data-target="#delete-modal-preview-{{ $leadership->id }}"> <i data-feather="trash-2"
                                            class="w-4 h-4 mr-1"></i> Delete </a>
                                </div>
                            </td>
                        </tr>
                        <div class="modal" id="delete-modal-preview-{{ $leadership->id }}">
                            <div class="modal__content modal__content--xl p-10">
                                ...
                                <div class="px-5 pb-8 text-center">
                                    <form action="{{ route('admin.leadership.destroy', $leadership->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div class="modal" id="notice-content-{{ $leadership->id }}">
