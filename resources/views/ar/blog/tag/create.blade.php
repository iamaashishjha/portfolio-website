                    ...
                    <div class="preview">
                        ...
                        <div class="mt-5 mb-5">
                            <div class="w-full sm:w-auto flex items-center sm:ml-auto mt-3 sm:mt-0">
                                ...
                                <input class="show-code input input--switch border @error('status') border-theme-6 @enderror" type="checkbox" name="status" {{ isset($tag) ? $tag->checkStatus() : '' }}>
                            </div>
                        </div>
                        <div class="mt-3">
                            <h5 class="text-lg ext-theme-9 @error('tag_image') text-theme-6 @enderror font-medium leading-none">Tag Image Upload <span class="text-lg ext-theme-9 text-theme-6 font-medium leading-none">*</span></h5>
                            <div class="border-2 border-dashed rounded-md mt-2 pt-4 @error('tag_image') border-theme-6 @enderror">
                                <div class="flex flex-wrap px-4">
                                    <div class="w-24 h-24 relative image-fit mb-5 mr-5 cursor-pointer zoom-in">
                                        <img class="rounded-md" alt="{{ isset($tag) ? $tag->description : '' }}" src="{{ isset($tag) ? $tag->image : '/ar/dist/images/preview-6.jpg' }}" id="tag-image">
                                    </div>
                                </div>
                        ...
                        <div class="mt-3">
                            ...
                            <input type="text" id="slug" name="slug" class="input w-full border mt-2 cursor-not-allowed  bg-gray-100" placeholder="Enter Slug" value="{{ isset($tag) ? $tag->slug : old('slug') }}" readonly>
                        </div>
                        <div class="mt-3">
                            <label>Tag Meta Description</label>
                            <input type="text" id="meta_description" name="meta_description" class="input w-full border mt-2 cursor-not-allowed bg-gray-100" placeholder="Enter Meta Description" value="{{ isset($tag) ? $tag->meta_description : old('meta_description') }}" readonly>
                        </div>
                        <div class="mt-3">
                            <label>Tag Keywords</label>
                            <input type="text" id="keywords" name="keywords" class="input w-full border mt-2 cursor-not-allowed bg-gray-100" placeholder="Enter Keywords" value="{{ isset($tag) ? $tag->keywords : old('keywords') }}" readonly>
                        </div>

                        <div class="mt-3">
                            <label>Tag Meta Title</label>
                            <input type="text" id="meta_title" name="meta_title" class=" cursor-not-allowed input w-full border mt-2 bg-gray-100" placeholder="Enter Meta Title" value="{{ isset($tag) ? $tag->meta_title :old('meta_title') }}" readonly>
                        </div>
                        <div class="mt-6 text-center">
                            <button type="submit" class="button w-50 mr-1 mb-2 {{ isset($tag) ? 'bg-theme-3 text-white' : 'bg-theme-9 text-white' }}">{{ isset($tag) ? 'Update Tag' : 'Create New Tag' }}</button>
                        </div>
                    </div>
                </div>
            </form>
