...
{{ isset($library) ? 'Edit Library ' . '"' . $library->title . '". | Nagrik Unmukti Party' : 'Create New Library |
...
<div class="intro-y flex items-center mt-8 ">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
        <a class="button text-white bg-theme-9 shadow-md mr-2" href="{{ route('admin.library.index') }}">All
            Library</a>
    </div>
</div>
                ...
                <div class="p-5" id="input">
                    ...
                    <div class="mt-5">
                        ...
                        <input type="text" name="meta_description" class="input w-full border mt-2"
                            ...
                            value="{{ isset($library) ? $library->meta_description : old('meta_description') }}" required>
                    </div>
                    <div class="mt-5">
                        <h5
                        ...
                        <input type="text" name="keywords" class="input w-full border mt-2" placeholder="Enter Keywords"
                            value="{{ isset($library) ? $library->keywords : old('keywords') }}" required>
                    </div>
                    <div class="mt-6 text-center">
                        <button type="submit"
                            class="button w-50 mr-1 mb-2 {{ isset($library) ? 'bg-theme-3 text-white' : 'bg-theme-9 text-white' }}">{{
                            isset($library) ? 'Update Library' : 'Create New Library' }}</button>
                    </div>
                </div>
        </div>
        </form>
...
<script>
    var loadImage = function(event) {
        var image = Library.getElementById('image');
        image.src = URL.createObjectURL(event.target.files[0]);
    };

    var loadFile = function(event) {
        var image = Library.getElementById('file');
